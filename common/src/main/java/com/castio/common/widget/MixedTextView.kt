...
class MixedTextView : AppCompatTextView {
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        mExactly = widthMode == MeasureSpec.EXACTLY
    }

    override fun setText(text: CharSequence, type: BufferType) {
        if (width > 0 && height > 0 && maxLines > 1 && mEnabled) {
            val newText = autoSplitText(this, text)
    ...
    private fun autoSplitText(tv: TextView, text: CharSequence): String {
        ...
        //控件可用宽度
        val maxWidth: Int = if (!mExactly) {
            tv.maxWidth
        } else {
            width
        }
        val tvWidth = (maxWidth - tv.paddingLeft - tv.paddingRight).toFloat()
